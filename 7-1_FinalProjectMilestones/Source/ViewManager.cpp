...
namespace
{
	...
	float gDeltaTime = 100.0f; 
	float gLastFrame = 100.0f;
...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	// callback to receive mose scroll callback
...
void ViewManager::Mouse_Position_Callback(GLFWwindow* window, double xMousePos, double yMousePos)
{
	...
	// sensitivity value for the mouse movement
	float sensitivity = 0.8f;
	xOffset *= sensitivity;
	yOffset *= sensitivity;

	// calculate the new yaw and pitch values for the camera
	float yaw = xOffset;
	float pitch = yOffset;
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	//process camera perspective projections
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS)
	{
		...
		g_pCamera->Position = glm::vec3(0.0f, 5.0f, 8.0f);
		g_pCamera->Front = glm::vec3(0.0f, -0.5f, -2.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
		g_pCamera->Zoom = 80;
	}
